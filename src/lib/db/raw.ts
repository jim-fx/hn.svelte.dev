...
export function getRawCache(path: string): ReturnType<typeof deserializeRow> | undefined {
	return db.run<RawRow>(statements.select_raw, { deserialize: deserializeRow }).get({ path });
}
