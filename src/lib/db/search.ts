...
export async function searchItems(query: string, searchInBody: boolean = false) {
  ...
  const searchSql = useFTS
    ? searchInBody ? statements.search_story_body : statements.search_story
    : searchInBody ? statements.search_story_body_like : statements.search_story_like;
  const rows = db.run<SearchItemRow>(searchSql).all({ query: useFTS ? query : `%${query}%`, limit });
...
export async function searchUsers(query: string, searchInAbout: boolean = false): Promise<User[]> {
  ...
  const searchQuery = useFts
    ? searchInAbout ? statements.search_user_about: statements.search_user
    : searchInAbout ? statements.search_user_about_like : statements.search_user_like;

  const rows = db.run<SearchUserRow>(searchQuery).all({ query: useFts ? query : `%${query}%`, limit });
  const ids = [...new Set(rows.map((row) => row.id))];
...
export async function searchComments(query: string): Promise<Item[]> {
	const limit = 50;
	let items: (Item | undefined)[];
  const searchSql = query.length < 3 
    ? statements.search_comment_like
    : statements.search_comment;
