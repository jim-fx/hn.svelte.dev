...
export type ExtendedDatabase = DatabaseSync & {
  ...
  run:<T = Record<string, SQLOutputValue>>(statement:string, options?: RunOptions<T>) =>  {
    all: {
...
export function openDatabase(dbName: 'hn.sqlite' | 'search.sqlite' | "statistics.sqlite", dbOpts?: DatabaseOptions): ExtendedDatabase {
      ...
      return {
        ...
        get: (...inputs:(SQLInputValue|Record<string,SQLInputValue>)[]) => {
          ...
          try {
            ...
            dbOpts?.queryCallback?.({sql: statement.sourceSQL, duration: performance.now()-start})
          }
        },
